...
COPY --chown=${USER_ID}:${GROUP_ID} ./packages/backend ${HOME}/packages/backend/
COPY --chown=${USER_ID}:${GROUP_ID} ./infra/env_vars/ ${HOME}/infra/env_vars/
COPY --chown=${USER_ID}:${GROUP_ID} ./infra/build_artifacts/dockerBackend-entrypoint.sh ${HOME}/scripts/
...
RUN chmod +x ${HOME}/scripts/dockerBackend-entrypoint.sh
...
RUN /bin/bash -c 'cd ${HOME}/packages/backend; npm ci; npm run build'


# Expose the port for backend api (4000) and gameserver (9000)
EXPOSE 4000
EXPOSE 5000

# set entrypoint to a script i wrote
ENTRYPOINT ["./scripts/dockerBackend-entrypoint.sh"]
